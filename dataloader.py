...
class IEMOCAPDataset(Dataset):
    ...
    def collate_fn(self, data):
        ...
        return [pad_sequence(dat[i].tolist()) if i<4 else pad_sequence(dat[i].tolist(), True) if i<6 else dat[i].tolist() for i in dat]
...
class AVECDataset(Dataset):
    ...
    def collate_fn(self, data):
        dat = pd.DataFrame(data)
        return [pad_sequence(dat[i].tolist()) if i<4 else pad_sequence(dat[i].tolist(), True) for i in dat]
