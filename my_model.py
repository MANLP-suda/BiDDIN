...
class DialogueRNNTriCell(nn.Module):
    ...
    def forward(self, Ut, Uv, Ua, qmask, g_hist_t, g_hist_v, g_hist_a, q0_t, q0_v, q0_a, e0_t, e0_v, e0_a,k=1):
        ...
        if g_hist_v.size()[0]==0:
            ...
            c_vv, alpha_vv = self.attention(g_hist_v,Uv)
            ...
            #T & V
            c_vt, alpha_vt = self.attention(g_hist_t,Uv)
            c_tv, alpha_tv = self.attention(g_hist_v,Ut)
            
            #A & V
            c_va, alpha_va = self.attention(g_hist_a,Uv)
            c_av, alpha_av = self.attention(g_hist_v,Ua)
...
class AVECBiModelTri(nn.Module):
    ...
    def __init__(self, D_m_T, D_m_V, D_m_A, D_m, D_g, D_p, D_e, D_h, attr, listener_state=False,
        ...
        self.matchatt = MatchingAttention(6*D_e,6*D_e,att_type='general2')
