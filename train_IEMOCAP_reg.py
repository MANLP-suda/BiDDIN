...
from model import AVECModel, MaskedMSELoss,myRTModelReg
# from dataloader import AVECDataset
from utils_reg import AVECDataset
...
def get_AVEC_loaders(path, dim=0, batch_size=32, valid=0.1, num_workers=0, pin_memory=False):
    trainset = AVECDataset(path=path,dim=dim)
    ...
    testset = AVECDataset(path=path, dim=dim, train=False)
...
#        print ('textf type',type(textf))                         
        # pred = model(torch.cat((textf,visuf,acouf),dim=-1), qmask) # batch*seq_len
        pred = model(torch.cat((textf,acouf),dim=-1), qmask) # batch*seq_len
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--dim', type=int, default=0, help='iemocap attribute')
    parser.add_argument('--dim', type=int, default=0,
                        help='emotion dimension')
    ...
    model = myRTModelReg(D_m, D_g, D_p, D_e, D_h,
                    listener_state=args.active_listener,
                    context_attention=args.attention,
                    dropout_rec=args.rec_dropout,
                    dropout=args.dropout)
    ...
    #                             num_workers=2)
    train_loader, valid_loader, test_loader =\
            get_AVEC_loaders('./DialogueRNN_features/DialogueRNN_features/IEMOCAP_features/IEMOCAP_features_raw.pkl',
                                dim=args.dim,
                                valid=0.0,
                                batch_size=32,
                                num_workers=2)
