...
def toRegScore(original,info,dim):
    labels = {}
    for idx in original.keys():
        if len(original[idx])==len(info[idx]):
            labels[idx] = [x[dim] for x in info[idx]]
        elif len(original[idx])>len(info[idx]):
            print ('idx, orig and info',idx,len(original[idx]),len(info[idx]))
            labels[idx] = [x[dim] for x in info[idx]]
            for i in range(len(original[idx])-len(info[idx])):
                labels[idx].append(2.000)
        else:
            print ('idx, orig and info',idx,len(original[idx]),len(info[idx]))
            labels[idx] = [x[dim] for x in info[idx][:len(original[idx])]]
    return labels

class IEMOCAPDataset(Dataset):

    def __init__(self, path, dim, train=True):
        self.videoIDs, self.videoSpeakers, self.videoLabels, self.videoText,\
        self.videoAudio, self.videoVisual, self.videoSentence, self.trainVid,\
        self.testVid = pickle.load(open(path, 'rb'), encoding='latin1')

        info = pickle.load(open('/home/dzhang/IJCAI/2020/iemocap_raw/iemocap_raw_vid2labels2.p','rb'))
        self.videoLabels = toRegScore(self.videoLabels,info,dim)

        # print ('label type',type(self.videoLabels))
        # for item in self.videoLabels.keys():
        #     print ('key',item,'value',self.videoLabels[item])
        #     break
        # print ('Speakers type',type(self.videoLabels))   
        # print ('speakers',self.videoSpeakers[item])
        # print ('test1')
        '''
...
class AVECDataset(Dataset):
    ...
    def __init__(self, path, dim, train=True):
        ...
        # # print ('Speakers type',type(self.videoLabels))   
        # # print ('speakers',self.videoSpeakers[item])
        # print ('text type',type(self.videoSentence))   
        # print ('text',self.videoSentence[item])
